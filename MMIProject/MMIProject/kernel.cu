 ...
 * 
 * The ground truth is assumed as the CPU implementation of the k-NN.
 *
...
bool test(const float* ref,
    ...
    float* test_knn_dist = (float*)malloc(query_num * k * sizeof(float));
    int* test_knn_index = (int*)malloc(query_num * k * sizeof(int));
    ...
    for (int i = 0; i < nb_iterations; ++i) {
        if (!knn(ref, ref_num, query, query_num, dim, k, test_knn_dist, test_knn_index)) {
            free(test_knn_dist);
            free(test_knn_index);
            return false;
        }
    ...
    // Elapsed time in ms
    double elapsed_time;
    elapsed_time = ((double)(end_cpu - start_cpu));
    ...
    // Compute accuracy
    float precision_accuracy = nb_correct_precisions / ((float)query_num * k);
    float index_accuracy = nb_correct_indexes / ((float)query_num * k);
...
int main(void) {
    ...
    // Parameters
    const int ref_num = 20000;
    const int query_num = 20000;
    const int dim = 32;
    const int k = 128;
    ...
    //test(ref, ref_num, query, query_num, dim, k, knn_dist, knn_index, &knn_cuda_global, "knn_cuda_global", 1);
    test(ref, ref_num, query, query_num, dim, k, knn_dist, knn_index, &knn_cuda_texture, "knn_cuda_texture", 1);
